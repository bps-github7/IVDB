...
function generateJwt(projectId, user, signInProvider, expiresInSeconds, extraClaims = {}) {
    ...
    const customPayloadFields = Object.assign(Object.assign(Object.assign({ name: user.displayName, picture: user.photoUrl }, customAttributes), extraClaims), { email: user.email, email_verified: user.emailVerified, phone_number: user.phoneNumber, provider_id: signInProvider === "anonymous" ? signInProvider : undefined, auth_time: user.lastLoginAt != null
            ? utils_1.toUnixTimestamp(new Date(user.lastLoginAt))
            : user.lastRefreshAt != null
                ? utils_1.toUnixTimestamp(new Date(user.lastRefreshAt))
                : utils_1.toUnixTimestamp(new Date()), user_id: user.localId, firebase: {
            identities,
